# Theorem:
In the Parallel pairs protocol with m=4 parties and n pads, no two parties ever write to the same pad index
...
Parties A and B are restricted to Zone1, while parties C and D are restricted to Zone2
...
## Case 2: Intra-zone collisions (within Zone1)
...
# Conclusion:
Since all possible collision cases (inter-zone and both intra-zone cases) are proven impossible, the protocol guarantees that no two parties ever write to the same pad index.
